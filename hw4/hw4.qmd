...
mimic_icu_cohort_pre <- mimic_icu_cohort %>%
  ...
  select(-admit_provider_id, -subject_id, -hadm_id, -stay_id, -anchor_age, 
         -anchor_year, -intime, -outtime,
          -admittime, -dischtime, -deathtime, -edregtime, -anchor_year_group,
         -edouttime)
  format_race <- function(x) {
  gtsummary::fmt_number(x, decimals = 1, suffixing = c(" (%)"))
  }
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("variable", "Variable of interest", choices = names(mimic_icu_cohort)),
      checkboxInput("outliers", "Remove outliers in IQR method for measurements?", FALSE)
    ),
    mainPanel(
      tabsetPanel(
        tabPanel("Patient characteristics", plotOutput("variablePlot")),
...
server <- function(input, output) {
  ...
  output$variablePlot <- renderPlot({
    ...
    if(input$outliers) {
      # Assume that we are dealing with a numeric variable and remove outliers
      data <- data %>% filter(between(!!sym(input$variable), quantile(!!sym(input$variable), 0.25), quantile(!!sym(input$variable), 0.75)))
    }
