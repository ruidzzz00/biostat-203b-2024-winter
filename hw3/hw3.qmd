...
#|eval = true
# Import chartevents
chartevents_tble <- read_csv("~/mimic/icu/chartevents.csv.gz") %>%
...
# Join with icustays data
chartevents_icustays <- chartevents_filtered %>%
  inner_join(icustays_tble, by = c("subject_id", "hadm_id", "stay_id")) %>%
  filter(charttime >= intime & charttime <= outtime)

# Select first measurement within ICU stay
chartevents_first_meas <- chartevents_icustays %>%
  group_by(subject_id, stay_id, itemid) %>%
  summarise(value = first(value, order_by = charttime)) %>%
  ungroup()

# Pivot to the required format
chartevents_tble <- chartevents_first_meas %>%
  pivot_wider(
...
final_mimic_icu_cohort <- mimic_icu_cohort %>%
  ...
  left_join(labevents_tble, by = "hadm_id") %>%
  left_join(chartevents_tble, by = "hadm_id") %>%
  select(subject_id, hadm_id, icustay_id, intime, outtime, first_careunit, last_careunit, los, gender, anchor_age, anchor_year, anchor_year_group, dod) %>%
  print(width = Inf)
```
